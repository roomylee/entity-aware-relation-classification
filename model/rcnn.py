...
class TextRCNN:
    def __init__(self, sequence_length, num_classes, vocab_size, word_embedding_size,
                 cell_type, hidden_size, context_embedding_size=300, l2_reg_lambda=0.0):
        ...
        with tf.name_scope("text-representation"):
            self.y2 = tf.layers.dense(self.x, hidden_size, activation=tf.nn.relu,
        ...
        with tf.variable_scope('dropout'):
            self.y2 = tf.nn.dropout(self.y2, self.dropout_keep_prob)

        with tf.name_scope("max-pooling"):
            # self.y3 = tf.reduce_max(self.y2, axis=1)
            self.y3, self.alphas = attention(self.y2, attention_size=50, return_alphas=True)
